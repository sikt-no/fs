...
## Scope-avgrensning (besluttet av bruker 2026-08-20, revidert samme dag)

**I scope: BRU-PER-GRU-001–004 + GRU-006** — det frontend-prototypen implementerer, pluss
tidsbegrensning på tildelinger (GRU-006 ble først utsatt, deretter lagt tilbake i scope samme dag;
datamodellen støtter allerede perioder via `tstzrange`, så tillegget er API-flate + nytt UI).
...
Fra GRU-006 (nå i scope — disse må avklares før/under implementasjon):
- Kan tidsbegrensning settes på rolle-nivå, eller bare på enkelttilgang og på det samlede tildelingssettet?
- Skal starttidspunkt i fortiden tillates (registrere historisk gyldighet), eller avvises som sluttidspunkt i fortiden?
- Skal «tidsbegrensning utløpt» og «manuelt deaktivert» vises som ulike statuser? (Henger sammen med GRU-002s åpne spørsmål.)
- Varsling av personbrukeren før utløp avklares sammen med BRU-PER-HIS-003 (historikk-iterasjonen — ikke i scope her).
