...
> Scope: BRU-PER-GRU-001–004 + GRU-006 (tidsbegrensning; lagt tilbake i scope 2026-08-20). Se
...
- **Vertikal B — tildelinger.** Eier `subjektrolletildeling`-flaten: visning av tilganger/roller,
  tildel/fjern-mutasjonene og tidsbegrensning på tildelinger (GRU-006). Bygger på
  fase4-tildel-fjern-maskineriet.
...
8. **Tidsbegrensning (GRU-006) bygger direkte på eksisterende `gyldig_periode` (tstzrange).**
   - Ingen nye kolonner: `gyldigFra`/`gyldigTil` eksponeres som nedre/øvre grense av
     `subjektrolletildeling.gyldig_periode` via arv-viewet; tildeling med tidsrom = insert med
     satt periode; «automatisk inaktiv/aktiv» faller ut av at tilgangsoppslagene allerede
     evaluerer perioden mot `now()`.
   - Temporal-immutabilitets-triggeren (0002) styrer endringsflatene: **sluttidspunkt** kan
     endres/fjernes direkte (øvre grense kan flyttes til `now()` eller senere, eller settes
     ubegrenset); **starttidspunkt** er låst etter innsetting — endring implementeres som
     lukk + ny rad (gir korrekt sporbarhet gratis).
   - «Felles sluttidspunkt for hele tildelingssettet» = én DB-funksjon som setter samme øvre
     grense på alle brukerens aktive direkte tildelinger, sporbart per rad (kravets ordlyd).
   - Validering i DB (egen SQLSTATE → typed feil `UgyldigTidsbegrensning`): sluttidspunkt i
     fortiden avvises; sluttidspunkt før starttidspunkt avvises. Tidspunkt tolkes i
     Europe/Oslo med minuttoppløsning (kravets Bakgrunn) — normaliseres til `timestamptz` i API-et.

9. **Ingen `kan*`-flagg, ingen `mineTilganger` i denne planen.**
...
- `tilgangsstyring-new-db/src/main/resources/db/changelog/0037-tidsbegrensning-brukertilganger.sql` — NY (Vertikal B, GRU-006)
- `tilgangsstyring-new-db/src/main/resources/db/changelog/db-changelog-root.xml` — 4 nye includes (koordinert)
- `tilgangsstyring-new-db/src/test/sql/30_deaktivering_feide_bruker.sql`, `32_rolletype_og_tildelingsvisning.sql`, `33_tildel_og_fjern_brukertilganger.sql`, `34_tidsbegrensning_brukertilganger.sql` — NYE pgTAP-suiter
...
- 6 nye mutations (`tildelFeideBrukerTilganger`, `fjernFeideBrukerTilganger`, `deaktiverFeideBrukere`, `reaktiverFeideBrukere`, `oppdaterFeideBrukerTildelingPerioder`, `settFeideBrukerFellesSluttidspunkt`)
- 1 ny node-type (`FeideBrukerTildeling`, typeId 20019), 1 nytt felt på `Tilgangsrolle` (`rolletype`), 8 inputs, 6 payloads, 5 error-unions, 2 nye feiltyper (`FeideBrukerIkkeFunnet`, `UgyldigTidsbegrensning`)
    ...
    #   keyColumns: ["SUBJEKT_ID","ROLLEKODE","ORGANISASJONSKODE","MILJOKODE"])  — firedelt nøkkel som fase4
{
  ...
  "Starttidspunkt for gyldighet (nedre grense av gyldig_periode). Null = uten startbegrensning."
  gyldigFra: DateTime                 # @field — ny view-kolonne (GRU-006)
  "Sluttidspunkt for gyldighet (øvre grense av gyldig_periode). Null = uten sluttbegrensning."
  gyldigTil: DateTime                 # @field — ny view-kolonne (GRU-006)
}
...
input TildelFeideBrukerTilgangerInput {
  ...
  "Valgfri tidsbegrensning (GRU-006). Utelatt = gjelder fra nå, uten sluttidspunkt."
  gyldigFra: DateTime
  gyldigTil: DateTime
}
...
union TildelFeideBrukerTilgangerError = ManglerTildelingsrett | FeideBrukerIkkeFunnet | TilgangskodeIkkeFunnet | UgyldigTidsbegrensning
...
#### Op #5: `OppdaterFeideBrukerTildelingPerioder` / `SettFeideBrukerFellesSluttidspunkt` — tidsbegrensning

**Dekker krav:** BRU-PER-GRU-006
**Implementeres av:** Task #13 (DB), Task #14 (skjema/service), Task #15 (fs-admin)
...
extend type Mutation {
  """
  Setter, endrer eller fjerner start-/sluttidspunkt på enkelt-tildelinger.
  Null i et felt = fjern den grensen. Endring av starttidspunkt på en løpende tildeling
  gjennomføres som lukk + ny tildeling (temporal sporbarhet), transparent for klienten.
  """
  oppdaterFeideBrukerTildelingPerioder(input: [OppdaterFeideBrukerTildelingPeriodeInput!]!): OppdaterFeideBrukerTildelingPerioderPayload
    # @service → FeideBrukerTilgangService.oppdaterFeideBrukerTildelingPerioder
  "Setter samme sluttidspunkt på alle personbrukerens aktive direkte tildelinger (GRU-006, sporbart per tildeling)."
  settFeideBrukerFellesSluttidspunkt(input: [SettFeideBrukerFellesSluttidspunktInput!]!): SettFeideBrukerFellesSluttidspunktPayload
    # @service → FeideBrukerTilgangService.settFellesSluttidspunkt
}

input OppdaterFeideBrukerTildelingPeriodeInput {
  tildelingId: ID!        # @nodeId(typeName: "FeideBrukerTildeling")
  gyldigFra: DateTime
  gyldigTil: DateTime
}

input SettFeideBrukerFellesSluttidspunktInput {
  feideBrukerId: ID!      # @nodeId(typeName: "FeideBruker")
  sluttidspunkt: DateTime!
}

type OppdaterFeideBrukerTildelingPerioderPayload {
  tildelinger: [FeideBrukerTildeling]
  errors: [OppdaterFeideBrukerTildelingPerioderError]
}
type SettFeideBrukerFellesSluttidspunktPayload {
  feideBrukere: [FeideBruker]
  errors: [SettFeideBrukerFellesSluttidspunktError]
}

union OppdaterFeideBrukerTildelingPerioderError = ManglerTildelingsrett | FeideBrukerIkkeFunnet | TilgangskodeIkkeFunnet | ArvetTilgangKanIkkeFjernes | UgyldigTidsbegrensning
union SettFeideBrukerFellesSluttidspunktError   = ManglerTildelingsrett | FeideBrukerIkkeFunnet | UgyldigTidsbegrensning

"Sluttidspunkt i fortiden, eller sluttidspunkt før starttidspunkt."
type UgyldigTidsbegrensning implements Error {
  path: [String!]!
  message: String!
}
# @error-handler: DATABASE, egen SQLSTATE (f.eks. "TS002") fra valideringen i DB-funksjonene
```

##### Lag B — fs-admin call-site

```ts
// src/domains/tilgangsstyring/features/PersonbrukerDetails/components/PersonbrukerTilganger/EndreTidsbegrensningModal/mutations.ts
export const OPPDATER_TILDELING_PERIODER = gql(/* GraphQL */ `
  mutation OppdaterFeideBrukerTildelingPerioder($input: [OppdaterFeideBrukerTildelingPeriodeInput!]!) {
    oppdaterFeideBrukerTildelingPerioder(input: $input) {
      tildelinger { ...TildelingRowFields }
      errors { ... on Error { message path } }
    }
  }
`)
// Radene viser gyldigFra/gyldigTil via TildelingRowFields (utvidet med feltene);
// arvede tildelinger tilbyr ikke endre-handling (tilknytning=ARVET).
```

##### Lag C — Begrunnelse

- **Dekker krav:** BRU-PER-GRU-006 (sette/endre/fjerne start- og sluttidspunkt per tildeling; felles sluttidspunkt for tildelingssettet; validering av tidspunkt; automatisk aktiv/inaktiv følger av at tilgangsoppslagene evaluerer `gyldig_periode` mot `now()`).
- **Form:** Toppnivåfelt på `Mutation` per `fs-sikt-no-producer-best-practice §Bare felt på Mutation-typen kan utføre endringer`; payload returnerer oppdatert tilstand per `graphql-learn-mutations`. Semantisk felt for kravets «felles sluttidspunkt for hele tildelingssettet» per `fs-sikt-no-producer-schema-design §Vi innfører gjerne egne felt og typer for semantisk nyttige data-uttrekk` — bevisst egen mutasjon i stedet for at klienten fanner ut N `oppdater…`-kall (atomisk + sporbar som én handling).
- **Navngiving:** `gyldigFra`/`gyldigTil` i naturlig ordrekkefølge per `fs-sikt-no-producer-naming §Bruk naturlig ordrekkefølge`; `sluttidspunkt` uten forkortelse per `§Unngå forkortelser`. Nullable grenser per `fs-sikt-no-producer-best-practice §Nullability` (null = ubegrenset).
- **Colocation-status:** Følger colocation — payload spreader `TildelingRowFields` (utvidet med periodefeltene) slik at fanene oppdateres fra cache.
- **Alternativer vurdert:** generisk `oppdaterFeideBrukerTildelinger`-mutasjon med alle felter — forkastet (perioder og fremtidige felt har ulik autorisasjon/semantikk; smale mutasjoner er lettere å resonnere om). Egne `sett`/`fjern`-mutasjoner per grense — forkastet (null-semantikken dekker fjerning; færre operasjoner).
- **DB-avgrensning (ingen ref dekker dette — dokumentert valg):** starttidspunkt-endring gjøres som lukk + ny rad fordi temporal-triggeren (0002) låser nedre grense; sluttidspunkt endres direkte (øvre grense kan flyttes til `now()` eller senere).
...
|---|---|---|
| `tildelFeideBrukerTilganger` | `TildelFeideBrukerTilgangerError` | `ManglerTildelingsrett`, `FeideBrukerIkkeFunnet`, `TilgangskodeIkkeFunnet`, `UgyldigTidsbegrensning` |
| `fjernFeideBrukerTilganger` | `FjernFeideBrukerTilgangerError` | `ManglerTildelingsrett`, `FeideBrukerIkkeFunnet`, `TilgangskodeIkkeFunnet`, `ArvetTilgangKanIkkeFjernes` |
| `oppdaterFeideBrukerTildelingPerioder` | `OppdaterFeideBrukerTildelingPerioderError` | som fjern + `UgyldigTidsbegrensning` |
| `settFeideBrukerFellesSluttidspunkt` | `SettFeideBrukerFellesSluttidspunktError` | `ManglerTildelingsrett`, `FeideBrukerIkkeFunnet`, `UgyldigTidsbegrensning` |
...
Kun `FeideBrukerIkkeFunnet` og `UgyldigTidsbegrensning` er nye; resten gjenbrukes fra main/fase4.
...
- [ ] **5. Starttidspunkt i fortiden (GRU-006).** Kravet avviser sluttidspunkt i fortiden eksplisitt, men lar starttidspunkt i fortiden stå åpent (registrere historisk gyldighet). Påvirker valideringen i Task #13. Kravansvarlig (#484). Antakelse inntil svar: tillates (perioden er faktahistorikk).

## Implementation Tasks

Rekkefølge/eierskap: Task #1 (grunnmur) landes først av utvikler B med review fra A. Deretter
kjører **Vertikal A = Task #2–#6 (utvikler A)** og **Vertikal B = Task #7–#11 + #13–#15
(utvikler B)** parallelt. Task #12 tas av den som blir ferdig først. jOOQ-regenerering er
serielt punkt: grunnmur → A → B, alltid på fersk main.
...
- [ ] Migrasjon `0034-rolletype-og-tildelingsvisning.sql`: `rolle.rolletype` m/ CHECK + seed-klassifisering; `CREATE OR REPLACE VIEW subjektrolletildeling_med_arv` m/ appendede kolonner `tildelt_av_bruker`, `tildelt_tidspunkt`, `rolletype`, `gyldig_fra`, `gyldig_til` (nedre/øvre grense av `gyldig_periode` — GRU-006)
...
- [ ] `tildel_brukertilgang` tar valgfrie `gyldig_fra`/`gyldig_til`-parametre og validerer dem (slutt i fortid / slutt før start → egen SQLSTATE, jf. `UgyldigTidsbegrensning`) — GRU-006
...
- [ ] Mutasjonene fra Op #3 i B-sonen m/ payloads, `FeideBrukerIkkeFunnet` gjenbrukt fra A (eller definert her hvis B lander først), fase4-feiltypene gjenbrukt; `TildelFeideBrukerTilgangerInput` m/ valgfrie `gyldigFra`/`gyldigTil` (GRU-006)
...
- [ ] Kolonner: kode, beskrivelse, tilknytning (m/ arvetFra-merking), organisasjon, miljø, tildelt av, tildelt dato, gyldighetstidsrom når satt (GRU-002/GRU-006)
...
**Priority**: Medium
**Size**: S
**Dependencies**: Task #5, #6, #10, #11, #15
...
### Task #13: Migrasjon 0037 — tidsbegrensning i DB (Vertikal B)

**Priority**: High
**Size**: M
**Dependencies**: Task #8. **Delvis blokkert av åpent spørsmål 5** (starttidspunkt i fortiden — antakelse: tillates)
**Addresses Requirements**: BRU-PER-GRU-006

**Acceptance Criteria**:

- [ ] `0037-tidsbegrensning-brukertilganger.sql`: `oppdater_brukertilgang_periode` (SECURITY INVOKER; endrer øvre grense direkte, null = ubegrenset; endret nedre grense implementeres som lukk + ny rad pga. temporal-triggeren i 0002) og `sett_felles_sluttidspunkt` (samme øvre grense på alle brukerens aktive direkte tildelinger)
- [ ] Validering med egen SQLSTATE (f.eks. `TS002`): sluttidspunkt i fortiden avvises, sluttidspunkt før starttidspunkt avvises
- [ ] Arvede tildelinger kan ikke få endret periode (samme vern som fjerning — arv følger opphavet)
- [ ] pgTAP `34_tidsbegrensning_brukertilganger.sql` grønn: tildeling m/ fremtidig start er inaktiv i tilgangsoppslag til starttidspunktet, utløpt tildeling gir ikke tilgang, endring/fjerning av grenser, felles sluttidspunkt treffer alle aktive og er sporbar per rad, valideringsfeilene, samspill m/ deaktivering (GRU-004: tidsutløpt under frys forblir utløpt)

**Implementation Notes**:
Ingen nye kolonner — alt bygger på `subjektrolletildeling.gyldig_periode`. Tilgangsoppslagene (`tilganger_for_*`) evaluerer allerede perioden mot `now()`, så «automatisk aktiv/inaktiv» krever ingen jobb/scheduler.

### Task #14: Skjema + service for periode-mutasjonene (Vertikal B)
...
- [ ] Op #5 i B-sonen: `oppdaterFeideBrukerTildelingPerioder`, `settFeideBrukerFellesSluttidspunkt`, `UgyldigTidsbegrensning` + payloads/unions; `gyldigFra`/`gyldigTil` eksponert på `FeideBrukerTildeling`
- [ ] `FeideBrukerTilgangService` utvidet med de to metodene (`ctx.transactionResult`, node-id-dekoding, kaller 0037-rutinene)
- [ ] IT: sette/endre/fjerne sluttidspunkt, starttidspunkt-endring gir ny tildeling m/ korrekt sporing, felles sluttidspunkt, valideringsfeil → `UgyldigTidsbegrensning` m/ korrekt `path`, arvet tildeling avvises

### Task #15: fs-admin — UI for tidsbegrensning (Vertikal B)

**Priority**: High
**Size**: L
**Dependencies**: Task #11, Task #14; designutkast (cross-contributor — UI-et finnes ikke i prototypen)
**Addresses Requirements**: BRU-PER-GRU-006 (+ GRU-002-scenarioet «Se tidsbegrensning på en tildeling»)

**Acceptance Criteria**:

- [ ] Tildel-modalen har valgfrie dato-/tidsvelgere for `gyldigFra`/`gyldigTil` (minuttoppløsning, Europe/Oslo)
- [ ] Fanene viser gyldighetstidsrom på tildelinger som har det, og skiller «utløpt på tid» fra aktive
- [ ] Endre-/fjern-tidsbegrensning-flyt per tildeling (kun DIREKTE) + «sett felles sluttidspunkt»-handling på brukernivå, koblet til Op #5-mutasjonene
- [ ] Klientvalidering speiler serverens regler (slutt i fortid, slutt før start) med feilmeldinger fra kravet
- [ ] Følger fs-admin-skills for inputs/modaler; jest-tester for valideringen
...
- **Risk**: Tidsbegrensnings-UI-et (Task #15) finnes ikke i prototypen — ingen design å migrere fra.
  - **Mitigation**: Designutkast bestilles tidlig (cross-contributor); backend (Task #13–#14) er uavhengig av designet og kan lande først. GRU-002-visningen (tidsrom i fanene) kan leveres før redigerings-UI-et.
- **Risk**: Temporal-triggeren gjør starttidspunkt-endring til lukk + ny rad — tildelings-id-en klienten holder blir foreldet.
  - **Mitigation**: Payloaden returnerer de nye tildelingene; fanene refetches/cache-oppdateres fra payload (dokumentert i Op #5).

### Testing Requirements

- pgTAP for alle nye DB-funksjoner/views/kolonner (suitene 30, 32, 33, 34) — kjøres via `mvn -pl :tilgangsstyring-new-db clean test`
...
- [ ] Alle akseptansekriterier i Task #1–#15 oppfylt
...
- [ ] Alle @must-krav i scope (GRU-001–004 + 006) adressert; utsettelsene (005, sist innlogget) dokumentert
...
| BRU-PER-GRU-002 | Se en personbrukers tilganger og roller | #1, #7, #10 | Planned (tidsromvisning dekket via #1/#10/#15; stedkodevisning utsatt m/ 005) |
...
| BRU-PER-GRU-006 | Tidsbegrensning for tildeling | #1, #8, #13, #14, #15 | Planned (lagt tilbake i scope 2026-08-20; åpent spørsmål 5 om start i fortiden) |
...
| Kravansvarlig (#484) | Starttidspunkt i fortiden — tillates eller avvises? | Task #13-validering (antakelse: tillates) |
| Design (fs-admin) | UI-utkast for tidsbegrensning (datovelgere, endre/fjerne-flyt, felles sluttidspunkt) | Task #15 (backend #13–#14 går uavhengig) |
