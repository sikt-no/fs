...
| GRU-006 tidsbegrensning | ❌ (nytt UI må designes — finnes ikke i prototypen) | ⚠️ `gyldig_periode` (tstzrange) finnes i tildelingstabellene; ingen API for å sette/endre — **i scope** (lagt tilbake 2026-08-20) |
...
- Temporal-immutabilitets-triggeren (`trig_enforce_temporal_immutability`, 0002) begrenser
  tidsbegrensnings-endringer (GRU-006): nedre grense (starttidspunkt) er låst etter innsetting,
  og øvre grense kan kun flyttes til `now()` eller senere. Å endre starttidspunkt på en
  eksisterende tildeling må derfor gjøres som lukk + ny rad; slutttidspunkt kan endres direkte.
...
| Design (fs-admin) | UI-design for tidsbegrensning (datovelgere i tildel-modal, endre/fjerne-flyt, felles sluttidspunkt, visning av tidsrom) | GRU-006 er i scope, men prototypen har ikke dette UI-et — designutkast trengs før frontend-tasken |
