...
- **Hva skal logges (K16):** Alle administrative handlinger, eller kun de sensitive (passord, tilganger, deaktivering)? Skal autentiseringshistorikk inngå i samme logg eller holdes atskilt?
...
- Den opprinnelige Iterasjon 4 hadde fire planlagte features (`oversikt_egne_applikasjoner`, `applikasjoner_med_tilgang_til_larested`, `selvbetjent_tildeling_tilgang`, `selvbetjent_fjerning_tilgang`). Disse er bevisst slått sammen med eksisterende features via rettighetsregler i stedet for å bli egne filer — det reduserer duplisering og holder logikken samlet med den generelle administrative flyten.
