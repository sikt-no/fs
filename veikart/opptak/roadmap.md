# Veikart — Opptak

Siste oppdatering: 2026-09-16

Dette veikartet viser oppgaver teamene aktivt jobber med eller har levert for samordna opptak 2027. For overordnet prioritering på tvers av FS, se [Forberede opptak og etterbehandling](https://github.com/orgs/sikt-no/projects/10/views/24).

## Initiativer

| Issue | Tittel | Team | Produksjonsfrist | Jira Epic |
|-------|--------|------|-----------------|-----------|
| [#211](https://github.com/sikt-no/fs/issues/211) | Opprette og vedlikeholde opptak | Shiitake | 1. nov 2026 | [TAKE-318](https://sikt.atlassian.net/browse/TAKE-318) |
| [#576](https://github.com/sikt-no/fs/issues/576) | Ferdigstilling av regelverk | Shiitake (restanser) | 1. nov 2026 | [TAKE-1](https://sikt.atlassian.net/browse/TAKE-1) |
| [#398](https://github.com/sikt-no/fs/issues/398) | Utdanningstilbud i opptak | Shinkansen | 1. nov 2026 | – |
| [#216](https://github.com/sikt-no/fs/issues/216) | Ferdigstilling av plasstildeling | – | 8. des 2026 | [TAKE-3](https://sikt.atlassian.net/browse/TAKE-3) |

## Aktive oppgaver

### Opptak (#211) — Shiitake, utforskning

| Issue | Oppgave | MoSCoW | Jira |
|-------|---------|--------|------|
| [#577](https://github.com/sikt-no/fs/issues/577) | Sette opptaksperiode | Must | [TAKE-319](https://sikt.atlassian.net/browse/TAKE-319) |
| [#578](https://github.com/sikt-no/fs/issues/578) | Konfigurere tidlig behandling og tilbud | Must | [TAKE-320](https://sikt.atlassian.net/browse/TAKE-320) |
| [#579](https://github.com/sikt-no/fs/issues/579) | Sette frister og tidsperioder | Must | [TAKE-321](https://sikt.atlassian.net/browse/TAKE-321) |
| [#214](https://github.com/sikt-no/fs/issues/214) | Sette fellestekster for søkere | Should | [TAKE-322](https://sikt.atlassian.net/browse/TAKE-322) |
| [#214](https://github.com/sikt-no/fs/issues/214) | Svarmeldingsmal | Should | [TAKE-323](https://sikt.atlassian.net/browse/TAKE-323) |

### Regelverk (#576) — Shiitake (restanser), utvikling

| Issue | Oppgave | MoSCoW | Jira |
|-------|---------|--------|------|
| [#580](https://github.com/sikt-no/fs/issues/580) | Fjerne poenglikhetsregel fra rangeringsregelverk | Must | [TAKE-324](https://sikt.atlassian.net/browse/TAKE-324) |
| [#581](https://github.com/sikt-no/fs/issues/581) | Avklare vitnemålskravkoder for fagbrev og svennebrev | Must | [TAKE-325](https://sikt.atlassian.net/browse/TAKE-325) |
| [#582](https://github.com/sikt-no/fs/issues/582) | Varsling og begrensning ved endring/deaktivering/sletting av regelverk | Should | [TAKE-326](https://sikt.atlassian.net/browse/TAKE-326) |

### Utdanningstilbud (#398) — Shinkansen, utforskning

Se [oppgavefil](oppgaver/utdanningstilbud/oppgave.md) for detaljer.

### Plasstildeling (#216) — utsatt til 8. des, utforskning

| Issue | Oppgave | MoSCoW | Jira |
|-------|---------|--------|------|
| [#583](https://github.com/sikt-no/fs/issues/583) | Beregne antall tilbud per utdanningskvote fra relativ fordeling | Must | [TAKE-327](https://sikt.atlassian.net/browse/TAKE-327) |
| [#584](https://github.com/sikt-no/fs/issues/584) | Innføre kompensasjonstilbud ved opprykk (supplering) | Must | [TAKE-328](https://sikt.atlassian.net/browse/TAKE-328) |
| [#585](https://github.com/sikt-no/fs/issues/585) | Vise resultatet til saksbehandler | Must | [TAKE-329](https://sikt.atlassian.net/browse/TAKE-329) |
| [#586](https://github.com/sikt-no/fs/issues/586) | Publisere resultatet til søkerne | Must | [TAKE-330](https://sikt.atlassian.net/browse/TAKE-330) |
| [#587](https://github.com/sikt-no/fs/issues/587) | Håndtere svar fra søker | Must | [TAKE-331](https://sikt.atlassian.net/browse/TAKE-331) |

### Vedtaksformidling — utforskning

Se [oppgavefil](oppgaver/vedtaksformidling/oppgave.md) for detaljer.

### Harde tidsrammer

| Dato | Hendelse |
|------|----------|
| 6. okt 2026 | Simulering av opptak |
| 15. nov 2026 | HK-dir starter oppretting av opptak |
| 16. nov 2026 | Infomøte med fagskoler om registrering |
| 19. nov 2026 | Simulering av opptak (mer funksjonalitet) |
| 5. des 2026 | Registreringsfrist for utdanninger i samordna opptak 2027 |

## Levert

### Opptak (#211)

| Oppgave | Levert |
|---------|--------|
| Opprette et opptak (samordnet eller lokalt) | 2026-09-14 |
| Invitere læresteder til samordnet opptak | 2026-09-14 |
| Konfigurere dokumentasjonskrav | 2026-09-14 |
| Konfigurere søknad (nummerserie, maks alternativer) | 2026-09-14 |

### Regelverk (#576)

| Oppgave | Levert |
|---------|--------|
| Etablere og forvalte en regelverkssamling | 2026-09-14 |
| Definere kompetansekrav | 2026-09-14 |
| Definere hvordan søkere rangeres (poengberegning) | 2026-09-14 |
| Definere kvotetyper, grunnlag og mangelkoder | 2026-09-14 |
| Koble regelverket til opptak og utdanningstilbud | 2026-09-14 |
| Regelverkssamling kan endres, deaktiveres og slettes | 2026-09-14 |

### Plasstildeling (#216)

| Oppgave | Levert |
|---------|--------|
| Legge til runder for plasstildeling i ett opptak | 2026-09-14 |
| Beregne plassflyt ved ledige plasser per utdanningskvote | 2026-09-14 |
| Starte en ny plasstildeling | 2026-09-14 |
| Gjennomføre plasstildeling | 2026-09-14 |