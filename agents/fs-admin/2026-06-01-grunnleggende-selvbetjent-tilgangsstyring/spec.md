...
enum Identitetsleverandor {
  FEIDE
  MASKINPORTEN
  FS  # Legacy — kan vises i listen og forvaltes, men kan ikke velges ved nyopprettelse.
}
...
type ApplikasjonerConnection {
  edges: [ApplikasjonerEdge!]!
  pageInfo: PageInfo!
  totalCount: Int!  # for "X av Y"-tekst nederst i listen
}

type ApplikasjonerEdge {
  cursor: String!
  node: Applikasjon!
}
...
type ApplikasjonTilgangerConnection {
  edges: [ApplikasjonTilgangerEdge!]!
  pageInfo: PageInfo!
  totalCount: Int!
}

type ApplikasjonTilgangerEdge {
  cursor: String!
  node: ApplikasjonTilgang!
}
...
- **`Connection`/`Edge`/`PageInfo`** følger Relay cursor-spec. `totalCount` er ikke standard-Relay, men er nødvendig for "X av Y"-tekst i UI (eksisterende fs-admin-pattern).
